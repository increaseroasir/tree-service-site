...
// (src/lib/lead.functions.ts) for JS submits, and POST /api/lead
...
import { postTrackingEvent, readCrmConfig, type CrmConfig } from "@/lib/tracking.server";
import { normEmail, normPhone, readMetaConfig, sendMetaLead, sha256 } from "./meta.server";
import { ALERTS, sendAlert } from "./alerts.server";
...
export const buildLeadEvent = (
  ...
  const payload = {
    ...
    properties: {
      deviceType: /Mobile|Android|iPhone/i.test(input.page.userAgent) ? "mobile" : "desktop",
      source: "ai_studio",
      projectId: cfg.projectId,
      formName: "Free Quote",
...
export const handleLead = async (raw: unknown, deps: LeadDeps = {}): Promise<LeadResult> => {
  ...
  if (!crm) {
    await sendAlert(
      ALERTS.LEAD_UNCONFIGURED,
      { leadUuid, path: input.page.path },
      { env, fetchImpl },
    );
