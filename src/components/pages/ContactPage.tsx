...
// The hash isn't available during prerender, so read it after mount to keep
// server and client HTML identical (no hydration mismatch).
const useServiceTypeFromHash = () => {
  const { hash } = useLocation();
  const [service, setService] = useState<string | undefined>(undefined);
  useEffect(() => {
    const key = (hash ?? "").replace(/^#/, "");
    setService(key ? CONTACT_PRESELECT[key] : undefined);
  }, [hash]);
  return service;
};
...
const ContactPage = () => {
  ...
  return (
    <PageLayout>
      ...
      <div className="max-w-[1120px] mx-auto px-5 md:px-6 pt-[56px] cv-auto">
