  ...
  <PageLayout>
    ...
    <div className="max-w-[1120px] mx-auto px-5 md:px-6 pt-5">
      ...
      <p className="text-[19px] leading-[1.55] text-[#454f4a] mt-4 max-w-[66ch]">
        It depends on six things, and most of them you can see from your driveway. Below is what
        moves the number so you know what you're paying for and can spot a quote that's missing
        something. For your actual price,{" "}
    ...
    <div className="max-w-[1120px] mx-auto px-5 md:px-6 pt-8">
      <div className="bg-[var(--primary)] text-white grid grid-cols-2 md:grid-cols-4">
        {ILLUSTRATIVE_RANGES.map((r, i) => (
          ...
          >
            <div
              className="text-[26px] md:text-[34px] font-bold text-[var(--accent)]"
              style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
            >
              {r.perFoot}
            </div>
      ...
      <p className="text-[15px] leading-[1.6] text-[#8e8878] mt-3.5">
        Relative cost bands (lowest → highest), not dollar quotes. A small tree in an open yard is
        the cheapest job we do; a big tree over a house that needs a crane, or a storm call-out, is
        the most expensive.
      </p>
    </div>
    ...
    <div className="max-w-[1120px] mx-auto px-5 md:px-6 pt-16 cv-auto">
      <h2
        className="text-[30px] md:text-[44px] font-bold uppercase inline-block pb-[10px] border-b-[3px] border-[var(--accent)]"
        style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-7">
        {COST_FACTORS.map((f) => (
          <div key={f.title} className="bg-white border border-border p-6 md:p-8">
            <h3
            ...
            <p className="text-base leading-[1.55] text-[#5d6862] mt-2">{f.body}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="bg-[var(--primary)] text-white p-7 md:p-8">
          ...
          <ul className="mt-4 flex flex-col gap-3 text-[17px] text-[#dbe8e1]">
            <li className="border-b border-[#2c6650] pb-2.5">Tree removal (per tree)</li>
            <li className="border-b border-[#2c6650] pb-2.5">Stump grinding (per stump)</li>
          ...
          <p className="text-sm text-[#9fb0a8] mt-4">
            You pick what you want. A quote that's one lump number is hiding something.
          </p>
        </div>
      ...
      <div className="bg-[var(--forest-dark)] text-white p-6 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        <div>
          ...
          <p className="text-[18px] leading-[1.55] text-[#b8c2bc] mt-3.5">
            Cost factors only get you so far. A photo or a 20-minute visit gets you a written,
            itemized price for your actual tree — free.
          </p>
