...
const TopBar = () => (
  <div className="bg-[var(--forest-dark)] text-[var(--cream)] text-[13px]">
    <div className="max-w-[1240px] mx-auto px-4 md:px-7 py-2 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-6 text-center md:text-left">
      ...
      <div className="hidden md:flex gap-[22px] items-center text-[#cfd6d1]">
        {TOPBAR_LINKS.map((l) => (
          <Link key={l.label} to={l.to} className="text-[#cfd6d1] hover:text-[var(--cream)]">
            {l.label}
          </Link>
        ))}
        <Link to={ROUTES.emergency} className="text-[#cfd6d1] hover:text-[var(--cream)]">
          Storm damage
        </Link>
        <a href={PHONE_HREF} className="text-[var(--cream)] font-semibold">
          Call · {PHONE}
        </a>
      </div>
    </div>
