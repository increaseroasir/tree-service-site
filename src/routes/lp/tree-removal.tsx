...
const Cta = ({ id }: { id: string }) => (
  <div className="bg-white text-[hsl(var(--foreground))] p-6 md:p-7 shadow-[0_20px_60px_rgba(0,0,0,0.25)]">
    <h2
    ...
    <FunnelLayout>
      <section className="bg-[hsl(var(--primary))] text-white">
        <div className="max-w-[1120px] mx-auto px-5 md:px-7 py-9 md:py-14 grid grid-cols-1 md:grid-cols-[1.1fr_0.9fr] gap-9 md:gap-12 items-start">
          <div>
            <span
              className="inline-block bg-[hsl(var(--accent))] px-3 py-[7px] text-sm font-bold uppercase tracking-[0.16em]"
              style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
            >
            ...
            <h1
              ...
              style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
            >
              Twin Cities homeowners are getting tree removal prices from a phone photo in under 24 hours
            </h1>
            <p className="text-[19px] leading-[1.5] text-[#d3ded8] mt-4 max-w-[46ch]">
              For single-family homes in the metro with a dead, leaning, or
              storm-damaged tree. Not for utility line clearing or trees in the
              boulevard.
            </p>
            <ul className="mt-6 flex flex-col gap-2.5 text-[18px] text-[#e2ebe6]">
              {BULLETS.map((b) => (
                <li key={b} className="flex gap-2.5">
                  <span className="mt-[9px] w-2 h-2 bg-[hsl(var(--accent))] flex-none" />
                  {b}
                </li>
              ))}
            </ul>
            <p className="text-base text-[#9fb0a8] mt-6">
              ...
              or{" "}
              <a href={SMS_HREF} className="underline">text a photo</a>.
            </p>
          </div>
          <Cta id="lp1" />
        </div>
      ...
      <section className="max-w-[1120px] mx-auto px-5 md:px-7 py-12">
        <h2
          className="text-[30px] md:text-[40px] font-bold uppercase inline-block pb-[10px] border-b-[3px] border-[hsl(var(--accent))]"
          style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
        >
      ...
      <section className="bg-[hsl(var(--forest-dark))]">
